...
/* ============================================================
   ...
   Verbatim TSX port of docs/kokikillara-porfolio/js/hero-ascii.jsx.
   ...
   Caption copy arrives via CMS-fed props (zero hardcoded copy).
...
export default function HeroAscii({word, sub, animate, onDone, capWho, capCmd, capFlag}: HeroAsciiProps) {
  const wrapRef = useRef<HTMLDivElement | null>(null)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)
  const mouseRef = useRef<{col: number; row: number; isMoving: boolean; idle: ReturnType<typeof setTimeout> | undefined}>({
    col: -999,
    row: -999,
    isMoving: false,
    idle: undefined,
  ...
  useEffect(() => {
    ...
    function updatePhysics() {
      const mx = mouseRef.current.col
      const my = mouseRef.current.row
      const moving = mouseRef.current.isMoving
      for (let k = 0; k < cells.length; k++) {
        const cell = cells[k]
    ...
    const onMove = (e: MouseEvent) => {
      ...
      mouseRef.current.idle = setTimeout(() => {
    ...
    const onLeave = () => {
      mouseRef.current.col = mouseRef.current.row = -999
      mouseRef.current.isMoving = false
    }
  ...
  return (
    <div className="ascii-hero" ref={wrapRef} aria-hidden="true">
      ...
      <div className="ascii-hero-cap">
        ...
        <span style={{color: 'var(--ink)'}}>{capCmd}</span>
        <span style={{color: 'var(--accent)'}}>{capFlag}</span>
